...
var enableHttps=0; // 0-不启用https 1-启用 https 2-http和https共存

var epHTTPPort=803; // eportal http 端口

var enHTTPSPort=804; // eportal https 端口

var domainName=''; // 强制跳转域名

var page_data_encrypt='1'; //页面传输数据是否加密 0-不开启,1-开启

var encryption_type='1'; // 数据加密类型 0-ip,1-密钥(默认ip)
...
if (port_mode === '1') {
  if (enableHttps === 1) {
    ...
    if (window.location.protocol !== 'https:' || (domainName && window.location.hostname != domainName) || window.location.port != enHTTPSPort || window.location.pathname.indexOf('/eportal/portal/index') < 0) {
      window.location = 'https://' + (domainName || window.location.hostname) + ':' + enHTTPSPort + '/eportal/portal/index' + window.location.search;
    }
  } else if (enableHttps === 0) {
    if (window.location.protocol !== 'http:' || (domainName && window.location.hostname != domainName) || window.location.port != epHTTPPort || window.location.pathname.indexOf('/eportal/portal/index') < 0) {
      window.location = 'http://' + (domainName || window.location.hostname) + ':' + epHTTPPort + '/eportal/portal/index' + window.location.search;
    }
  } else {
    if ((domainName && window.location.hostname != domainName) || (window.location.port != epHTTPPort && window.location.port != enHTTPSPort) || window.location.pathname.indexOf('/eportal/portal/index') < 0) {
      window.location = window.location.protocol + '//' + (domainName || window.location.hostname) + ':' + window.location.protocol === 'http:' ? epHTTPPort : enHTTPSPort + '/eportal/portal/index' + window.location.search;
    }
  }
} else {
  if (enableHttps === 1) {
    ...
    if (window.location.protocol !== 'http:' || (domainName && window.location.hostname != domainName)) {
      window.location = 'http://' + (domainName || window.location.hostname) + window.location.pathname + window.location.search;
    }
  } else if (enableHttps === 2) {
    if (domainName && window.location.hostname != domainName) {
      window.location = window.location.protocol + '//' + (domainName || window.location.hostname) + window.location.pathname + window.location.search;
    }
  }
}
      ...
      term.init(function () {
        ...
        me.getPageInfo(function () {
          ...
          if (!me._kind) {
            ...
            // me.kind = term.type == 1 ? 'pc_' + _kind : util.switchPageKind(term.type) + '_3' + _kind;
  ...
  getPageInfo: function (next) {
    ...
    util._jsonp({
      ...
      success: function (json) {
        ...
        term.enable_verify      = parseInt(json.data.enable_verify) || 0;          // 校验密码强度 0 停用 1启用
  ...
  firstRender: function () {
	...
	if ((term.redirect && term.redirect.indexOf('wechat_redirect') >= 0) || window.location.hash == '#wechat_redirect' || window.location.search.indexOf('wechat_redirect') >= 0) {
	  ...
	  var data = {
	    // 二维码数据
	    'login_method': page.login_method,
      'mac_type': term.type == 2 ? 1 : 0, //手机终端标记
	    'wlan_user_ip': term.ip,
    ...
    util._load('js', page.page_asset + 'js/all.js', function () {
      ...
      util._load('js', page.page_asset + 'js/jquery.i18n.js', function() {
        ...
        if (isJSMin) {
           ...
           */
          var jsFiles = ['a42.js', 'a43.js', 'a44.js', 'a45.js', 'a47.js', 'a48.js', 'a49.js', 'a50.js', 'a51.js','a52.js','a53.js','a58.js','a60.js','a61.js','a59.js'];
...
var util = {
  ...
  getTermType: function () { // 访问设备:0-其他；1-PC；2-手机；3-平板(横屏)；4-平板(竖屏)
    var iTermType = 0;
    ...
    var isIpad = /(?:iPad|PlayBook)/.test(ua) || (/(?:Macintosh)/.test(ua) && document.documentElement.clientWidth < 1200);
    ...
    //判断为平板
    if(isTablet){
      //UA识别成IPAD时，浏览器显示宽度小于高度则加载IPAD(竖屏)页
      if ( window.screen.width < window.screen.height){
        iTermType = 4;
      }else{
        iTermType = 3;
    ...
    //   if (navigator.userAgent.indexOf('tablePC') > 0 || navigator.userAgent.indexOf('HUAWEIBAH3-W59') > 0) { // 安卓
    //     iTermType = 3;
    //   }else{
    ...
    //
    //   if (navigator.userAgent.indexOf('Macintosh') > 0) { // 苹果
    //     iTermType = 3;
    ...
    //
    // //UA识别成IPAD时，浏览器显示宽度小于高度则加载IPAD(竖屏)页
    // if (iTermType == 3 && window.screen.width < window.screen.height){
    //     iTermType = 4;
    // }
  ...
  _jsonp: function (params) {
    ...
    //根据参数判断是否加密数据
    if(page_data_encrypt == 1 && params.url.indexOf('drcom/chkstatus') == -1 &&  params.url.indexOf('drcom/login') == -1 && params.url.indexOf('drcom/getipv6') == -1 && params.url.indexOf('drcom/logout') == -1 ){
      //加密传输的数据
      if(encryption_type == '1'){
        //加密传输的数据
        var keys = this.getkey(secret_key);//这里应该用终端实际IP,可从ss5登参数中获取
      }else{
        //加密传输的数据
  ...
  */
  ...
  checkPassStrength: function (pass) {	
		// 密码强度未启用则不检查
		if(!term.enable_verify) return true;
		
		// 检查密码长度
		if(pass.length<term.length_require) return false;
		...
		var temp_ary = scope.join('|').split('|');	
		for(var i=0; i<pass.length; i++){
			if (!temp_ary.includes(pass.charCodeAt(i).toString())) return false;	  
		}
    ...
    return level >= term.compose_require;
  },
  switchPageKind: function (term_type){
    var page_kind = '';
    switch (parseInt(term_type)) {
      case 1:
        page_kind = 'pc';
        break;
      case 2: 
...
var term = {
  ...
  enable_digital_char: 0, // 数字字符组合启用状态
  enable_upper_case: 0,   // 大写字母组合启用状态
  enable_lower_case: 0,   // 小写字母组合启用状态
  enable_special_char: 0, // 特殊符号组合启用状态
  digital_char_scope: '', // 数字字符组合范围
  upper_case_scope: '',   // 大写字母组合范围
  lower_case_scope: '',   // 小写字母组合范围
  special_char_scope: '', // 特殊符号组合范围
